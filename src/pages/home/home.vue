<template>
    <div class="home">
        <header1 title="晞景儿科互联网医院" />
        <pullDown :parent="this" :refName="'content'" :loading="loadFn" />
        <section class="content" ref="content">
            <div class="wrap" v-if="onOff">
                ...
                <ol class="flow">
                    <li v-for="(item,index) in ['选择医生','在线问诊','查看医嘱/处方','送药上门']" :style="{
                            backgroundImage:'url('+require(`images/icon/series/img${index+1}.png`)+')',
                        }">
                        {{item}}
                    </li>
                </ol>
                <router-link v-show="userId&&userId!='null'" class="haveNoEndInquiry" :to="{
                        path:'/user/myInquiry',
                    }">
                    发现您有未结束的问诊，去看看~
                </router-link>
                <div v-show="hotDept&&hotDept.length>0" class="nav">
                    <a v-for="(item,index) in hotDept" :style="{
                ...
                <div class="hotDoctor">
                    ...
                    <div class="main">
                        <doctorList :api="this.residentIndex" :onceLoad="true" :firstLoad="getHomeData" />
                    </div>
                </div>
                <router-link to="/user" class="inquiry"></router-link>
            </div>
        </section>
    </div>
</template>
